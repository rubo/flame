...
package flame.crypto
{
	...
	/**
	 * Computes the SHA-384 hash value for the input data. This class cannot be inherited.
	 * <p>The hash size for the SHA-384 algorithm is 384 bits.</p>
	 */
	public final class SHA384 extends HashAlgorithm
	{
		//--------------------------------------------------------------------------
	    //
	    //  Fields
		...
		private var _sha384State:Vector.<Int64> = new Vector.<Int64>(8, true);
		...
		/**
		 * Initializes an instance of SHA384. 
		 */
		public override function initialize():void
		{
			...
			_sha384State[0] = new Int64(-876896931, -1056596264); 
			_sha384State[1] = new Int64(1654270250, 914150663);
			_sha384State[2] = new Int64(-1856437926, 812702999);
			_sha384State[3] = new Int64(355462360, -150054599);
			_sha384State[4] = new Int64(1731405415, -4191439);
			_sha384State[5] = new Int64(-1900787065, 1750603025);
			_sha384State[6] = new Int64(-619958771, 1694076839);
			_sha384State[7] = new Int64(1203062813, -1090891868);
		}
		...
		protected override function hashFinal():ByteArray
		{
			...
			_hash.clear();
			_hash.writeInt(_sha384State[0].highBits);
			_hash.writeInt(_sha384State[0].lowBits);
			_hash.writeInt(_sha384State[1].highBits);
			_hash.writeInt(_sha384State[1].lowBits);
			_hash.writeInt(_sha384State[2].highBits);
			_hash.writeInt(_sha384State[2].lowBits);
			_hash.writeInt(_sha384State[3].highBits);
			_hash.writeInt(_sha384State[3].lowBits);
			_hash.writeInt(_sha384State[4].highBits);
			_hash.writeInt(_sha384State[4].lowBits);
			_hash.writeInt(_sha384State[5].highBits);
			_hash.writeInt(_sha384State[5].lowBits);
		...
		private function hashData(data:ByteArray, offset:int, count:int):void
		{
	        ...
	        {
	            a = _sha384State[0].valueOf();
	            b = _sha384State[1].valueOf();
	            c = _sha384State[2].valueOf();
	            d = _sha384State[3].valueOf();
	            e = _sha384State[4].valueOf();
	            f = _sha384State[5].valueOf();
	            g = _sha384State[6].valueOf();
	            h = _sha384State[7].valueOf();
	            ...
	            _sha384State[0] = _sha384State[0].add(a);
	            _sha384State[1] = _sha384State[1].add(b);
	            _sha384State[2] = _sha384State[2].add(c);
	            _sha384State[3] = _sha384State[3].add(d);
	            _sha384State[4] = _sha384State[4].add(e);
	            _sha384State[5] = _sha384State[5].add(f);
	            _sha384State[6] = _sha384State[6].add(g);
	            _sha384State[7] = _sha384State[7].add(h);
	        }
